  ...
  private def mainContent() = div(
    div(
      ...
      ".choose-data-file-panel" >>> span(
        span(DataStore.config.map(_.map(_.dataDirPath).getOrElse[String]("No data file, please choose one first!"))),
        Button("Choose data dir").onClick(_ => Entry.chooseDataFilePage().go())
      )
    ),
